...
async def run_agent(
    ...
    if file_path and not any(t == "perform_bom_extraction" for t, _, _ in extract_tool_calls_from_trajectory(trajectory)):
        from backend.src.tools.bom_extraction.bom_tool import perform_bom_extraction
        result = perform_bom_extraction(file_path)
        # Check for tuple return
        if isinstance(result, tuple):
             bom_res, pub_res = result
             if isinstance(bom_res, BillOfMaterials):
                bom_id = compute_bom_id(bom_res, source_document=file_path)
                app.state.boms[thread_key] = {"bom_id": bom_id, "bom": bom_res, "source_document": file_path}
                append_to_history(history, user_query="__BOM_EXTRACTED__", process_result=bom_res.model_dump_json())
                blocks.append(
                    build_bom_tool_block(bom_res, source_document=file_path, preview_image=pub_res, bom_id=bom_id, thread_id=thread_key)
                )
             else:
            ...
            bom_id = compute_bom_id(result, source_document=file_path)
            app.state.boms[thread_key] = {"bom_id": bom_id, "bom": result, "source_document": file_path}
            append_to_history(history, user_query="__BOM_EXTRACTED__", process_result=result.model_dump_json())
            blocks.append(
                build_bom_tool_block(result, source_document=file_path, bom_id=bom_id, thread_id=thread_key)
            )
        else:
            blocks.append(TextBlock(content=str(result)))
